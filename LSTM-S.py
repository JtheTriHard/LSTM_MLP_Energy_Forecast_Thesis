...
new_order = ['dataid','Hour','solar_fixed','Temperature']
df = df[new_order]
num_cols = ['solar_fixed','Temperature']
embed_cols = ['dataid','Hour']
...
for i in range(nseries):
    ...
    x_temp, y_temp = multivariate_data(df_temp,df_temp['solar_fixed'],0,None,lags,steps_ahead,single_step=False)
...
newlayer= tf.keras.layers.LSTM(64,activation='tanh',return_sequences=True,name='LSTM_1')(input_layer)
newlayer = tf.keras.layers.Dropout(0.1,name='Dropout_1')(newlayer,training=True)
newlayer= tf.keras.layers.LSTM(32,activation='tanh',name='LSTM_2')(newlayer)
newlayer = tf.keras.layers.Dropout(0.1,name='Dropout_2')(newlayer,training=True)
#newlayer= tf.keras.layers.LSTM(32,activation='tanh',name='LSTM_3')(newlayer)
#newlayer = tf.keras.layers.Dropout(0.1,name='Dropout_3')(newlayer)
#newlayer= tf.keras.layers.LSTM(32,activation='tanh',name='MLP_4')(newlayer)
output_layer = tf.keras.layers.Dense(steps_ahead,name='output_layer')(newlayer)
model = tf.keras.Model(inputs, output_layer, name = "LSTM-C-S")
model.compile(optimizer='adam',loss='mse')
model.summary()
tf.keras.utils.plot_model(model, 'YOUR_DIRECTORY', show_shapes=True)
history = model.fit([x_train[:,:,0],x_train[:,:,1],
                     x_train[:,:,2:]],y_train,batch_size=batchsize, epochs=n_epochs,
...
yhat_test = model.predict([x_test[:,:,0], x_test[:,:,1], x_test[:,:,2:]])
...
xtest_unscaled[:,:,2] = unscale(x_test[:,:,2], steps_ahead) # note that only target var is unscaled
...
def multiplot(past_obs, true_obs, predicted_obs):
    ...
    plt.ylabel('Solar PV Generation (kWh)')
    plt.xlabel('Time (hr)')
...
if steps_ahead == 1:
    ...
    for i in range(plot_days):
        multiplot(xtest_unscaled[i*steps_ahead,:,2],ytest_unscaled[i*steps_ahead,:],yhat_unscaled[i*steps_ahead,:])
    # Last week of forecasts
    for i in range(-plot_days,0):
        multiplot(xtest_unscaled[i*steps_ahead,:,2],ytest_unscaled[i*steps_ahead,:],yhat_unscaled[i*steps_ahead,:])
...
# Plot embeddings
# 2 = dataid, 3 = hour
plot_embeddings(2,nseries,1)
plot_embeddings(3,24,0)
...
for j in range(iterations):
    ...
    df_dist = pd.DataFrame(list(zip(df_t['solar_fixed'],
...
in_original = [x_original[:,:,0],x_original[:,:,1],x_original[:,:,2:]]
...
for i in range(nseries):
    interval_plot(y_original[i*PI_length:(i+1)*PI_length],predictions[i*PI_length:(i+1)*PI_length],term[i*PI_length:(i+1)*PI_length])
