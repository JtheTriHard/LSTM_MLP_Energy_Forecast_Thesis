...
new_order = ['dataid','Hour','solar_fixed','Temperature','Wind Speed','Dew Point','Visibility']
num_cols = ['solar_fixed','Temperature','Wind Speed','Dew Point','Visibility'] # don't forget to check train_input
cat_cols = ['dataid','Hour']
df = df[new_order]
...
for i in cat_cols:
    ...
    cat_input = tf.keras.layers.Dropout(0.1)(cat_input,training=True)
    cat_input = tf.keras.layers.Dense(neurons,activation='tanh')(cat_input)
    cat_input = tf.keras.layers.Dropout(0.1)(cat_input,training=True)
    cat_input = tf.keras.layers.Dense(neurons,activation='tanh')(cat_input)
    cat_input = tf.keras.layers.Dropout(0.1)(cat_input,training=True)
...
for j in num_cols:
    ...
    num_input = tf.keras.layers.Dropout(0.1)(num_input,training=True)
    num_input = tf.keras.layers.Dense(neurons,activation='tanh')(num_input)
    num_input = tf.keras.layers.Dropout(0.1)(num_input,training=True)
    num_input = tf.keras.layers.Dense(neurons,activation='tanh')(num_input)
    num_input = tf.keras.layers.Dropout(0.1)(num_input,training=True)
...
model = tf.keras.Model(inputs, output_layer, name = "MLP-C-S")
...
# Unscale data
# edit to work for single step
def unscale(result,steps):
    if steps == 1: #single step
        if result.shape != (len(result),1): # if inputs
            temp = result.flatten() # (obs,)
            temp = np.vstack([temp,temp,temp,temp,temp]) # (2, obs). Change if scaler was fit on more than TWO features
            ...
            temp = np.hstack([result,result,result,result,result]) # match to num of scaled features
            temp = scaler.inverse_transform(temp)
            temp = temp[:,0]
    else: #multi step
        temp = result.flatten()
        temp = np.vstack([temp,temp,temp,temp,temp]) # match to num of scaled features
